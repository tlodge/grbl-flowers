  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    // NOTE: Computes true distance from converted step values.
      ...
      if ( !(idx == A_MOTOR) && !(idx == B_MOTOR) ) {
        target_steps[idx] = 0;// lround(target[idx]*settings.steps_per_mm[idx]);
        block->steps[idx] = 0;//labs(target_steps[idx]-pl.position[idx]);
      }
      ...
      if (idx == A_MOTOR) {
        ...
        delta_mm = 0;//(target_steps[idx] - pl.position[idx])/settings.steps_per_mm[idx];
      }
    #else
    //tlodge added if idx < 2 and else - want target_steps[2] to be ignored (ie set Z to 0)
      if (idx <2){
